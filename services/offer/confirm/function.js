...
import { useConfirmOfferMutation } from "../api";
...
function useConfirmForm({ form }) {
  ...
  const [confirmOffer, { isLoading: confirming }] = useConfirmOfferMutation();
  useEffect(() => {
    if (form) {
  ...
  const onFinish = async (data) => {
    ...
    if (res?.data) {
      const cRes = await confirmOffer({ issues });
      console.log(cRes);
      if (cRes?.data) {
...
export const ShowEasyStep = (dispatch, setCurrent) => {
  Modal.info({
